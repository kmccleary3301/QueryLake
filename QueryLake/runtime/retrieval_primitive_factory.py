...
from QueryLake.runtime.retrieval_lanes import list_retrieval_lane_bindings, resolve_retriever_builder
from QueryLake.runtime.retrieval_primitives_legacy import CrossEncoderReranker, RRFusion, WeightedScoreFusion
...
def list_retriever_builders() -> Dict[str, RetrieverBuilder]:
    builders = dict(_RETRIEVER_BUILDERS)
    for primitive_id in list_retrieval_lane_bindings().keys():
        builders.setdefault(
            primitive_id,
            (lambda resolved_primitive_id: (lambda **kwargs: resolve_retriever_builder(resolved_primitive_id)(**kwargs)))(
                primitive_id
            ),
        )
    return builders
...
def build_retrievers_for_pipeline(
    *,
    ...
    for stage in pipeline.stages:
        ...
        if builder is None:
            builder = resolve_retriever_builder(str(stage.primitive_id), profile=profile)
