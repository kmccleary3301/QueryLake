class EventSource {
  ...
  open() {
    try {
      ...
      this._xhr.setRequestHeader('Cache-Control', 'max-age');
  ...
  _handleEvent(response) {
    ...
    for (let i = this.last_part_processed; i < parts.length; i++) {
    ...
    this.last_part_processed = parts.length-1;
  }
