...
function generate_component_exports(directory) {
  ...
  files.forEach(file => {
    ...
    if (stats.isDirectory()) {
      ...
      console.log(first_four_columns);
      let arguments = [];
        ...
        if (is_input) {
          ...
          if (find_demo)
            editor_switch_strings_input.push([
              `case "${file_name}":\n  return (\n    `,
              `<SKELETON_${component_name} configuration={info}>\n      {children}\n    </SKELETON_${component_name}>\n  );`
            ].join("").replaceAll(/^|\n/g, "\n    "));
             // else
          //    editor_switch_strings_input.push(`case "${file_name}":\n  return (\n    <${component_name} configuration={info}/>\n  );`.replaceAll(/^|\n/g, "\n    "));
        } else {
          switch_strings_display.push(switch_case);
          display_component_ids.push(file_name);
          if (find_demo)
            editor_switch_strings_display.push(`case "${file_name}":\n  return (\n    <SKELETON_${component_name} configuration={(info as displayMapping)}/>\n  );`.replaceAll(/^|\n/g, "\n    "));
      
          // else 
        }
        
        if (find_demo) {
          editor_import_strings.push(`import { SKELETON as SKELETON_${component_name} } from '@/${directory}/${file_name}';`);
        }
  ...
  return [import_strings, editor_import_strings, switch_strings, editor_switch_strings, type_file_generated];
}
...
const [import_strings, editor_import_strings, switch_strings, editor_switch_strings, type_file_generated] = generate_component_exports(COMPONENTS_DIRECTORY);
...
const new_file_editor_display = editor_display_template.replace(auto_imports_section_regex, editor_import_strings.join("\n")).replace(auto_switch_regex, editor_switch_strings.join("\n"));
