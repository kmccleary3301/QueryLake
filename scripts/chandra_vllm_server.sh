...
CHANDRA_VLLM_ALLOWED_LOCAL_MEDIA_PATH="${CHANDRA_VLLM_ALLOWED_LOCAL_MEDIA_PATH:-$REPO_ROOT/object_store}"
...
if [[ -n "$CHANDRA_VLLM_ALLOWED_LOCAL_MEDIA_PATH" ]]; then
  mkdir -p "$CHANDRA_VLLM_ALLOWED_LOCAL_MEDIA_PATH"
fi
...
start_server() {
  ...
  if [[ -n "$CHANDRA_VLLM_ALLOWED_LOCAL_MEDIA_PATH" ]]; then
    cmd+=(--allowed-local-media-path "$CHANDRA_VLLM_ALLOWED_LOCAL_MEDIA_PATH")
  fi
